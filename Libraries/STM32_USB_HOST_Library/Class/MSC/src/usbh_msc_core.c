...
{	 
     ...
     (pphost->device_prop.Itf_Desc[0].bInterfaceProtocol == MSC_PROTOCOL))
  {
    ...
    USBH_Open_Channel  (pdev,
                        MSC_Machine.hc_num_out,				//2
                        pphost->device_prop.address,		//1
                        pphost->device_prop.speed,			//1
                        EP_TYPE_BULK,
                        MSC_Machine.MSBulkOutEpSize);		//0x40  
    
    USBH_Open_Channel  (pdev,
                        MSC_Machine.hc_num_in,				//3
                        pphost->device_prop.address,		//1
                        pphost->device_prop.speed,			//1
                        EP_TYPE_BULK,
                        MSC_Machine.MSBulkInEpSize);    	//0x40
                                   ...
                                   void   *phost)
{
  ...
  {   
    ...
    case USBH_MSC_BOT_INIT_STATE:
      USBH_MSC_Init(pdev);
      //USBH_MSC_BOTXferParam.MSCState = USBH_MSC_BOT_RESET; //@STM32 office
