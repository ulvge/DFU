...
static void  USBH_ParseDfuDesc (USBH_DFUDesc_TypeDef *Dfu_descriptor,uint8_t *buf);
...
PC 向address 0发送USB协议规定的Get_Device_Descriptor命令，
以取得缺省控制管道所支持的最大数据包长度，并在有限的时间内等待USB设备的响应。
该长度包含在设备描述符的bMaxPacketSize0字段中，
其地址偏移量为7，所以这时主机只需读取该描述符的前8个字节。
...

{
                                  ...
                                  length)) == USBH_OK)
  {
    ...
    USBH_ParseCfgDesc (&phost->device_prop.Cfg_Desc,
                       phost->device_prop.Itf_Desc,		//数组已经是指针
                       &phost->device_prop.DFU_Desc,
      ...
      while (ptr < cfg_desc->wTotalLength ) 
      {
        ...
        if (pdesc->bDescriptorType   == USB_DESC_TYPE_INTERFACE) 
        {
          ...
          if((*((uint8_t *)pdesc + 3)) < 3)
          {
    		  ...
    		  if(temp_pif.bInterfaceClass == 0xFE){
    			  USBH_ParseInterfaceDesc (pif, (uint8_t *)&temp_pif); 
                  
                  pdesc = USBH_GetNextDesc((void* )pdesc, &ptr);
                  if (pdesc->bDescriptorType   == USB_DESC_TYPE_Functional_DFU) 
                  {  
                      USBH_ParseDfuDesc((USBH_DFUDesc_TypeDef*)dfu_desc, (uint8_t *)pdesc); 
                  }
    		  }
...
static void  USBH_ParseDfuDesc (USBH_DFUDesc_TypeDef *Dfu_descriptor,uint8_t *buf)
{
  Dfu_descriptor->bLength            = *(uint8_t  *) (buf + 0);
  Dfu_descriptor->bDescriptorType    = *(uint8_t  *) (buf + 1);
  Dfu_descriptor->bmAttributes.d8	 = *(uint8_t  *) (buf + 2)&0x07;
  Dfu_descriptor->wDetachTimeOut   	 = LE16 (buf + 3);
  Dfu_descriptor->wTransferSize   	 = LE16 (buf + 5);
}
