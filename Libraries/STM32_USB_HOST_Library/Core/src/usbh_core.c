...
#include "xprintf.h"
#define printf_usbh_core xprintf 
               ...
               USBH_Usr_cb_TypeDef *usr_cb)
{
  ...
  /* Hardware Init */
  USB_OTG_BSP_Init(pdev);  //init GPIO
...
extern uint8_t USBH_DFU_DecomposePacked(USB_OTG_CORE_HANDLE *pdev);
...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  case HOST_DEV_ATTACHED :
    ...
    phost->usr_cb->DeviceAttached();//USBH_USR_DeviceAttached
    phost->Control.hc_num_out = USBH_Alloc_Channel(pdev, 0x00);//USB_EP_DIR_OUT
    phost->Control.hc_num_in = USBH_Alloc_Channel(pdev, 0x80);  //USB_EP_DIR_IN
    ...
    if ( HCD_ResetPort(pdev) == 0)
    {
      ...
      */
      ...
      phost->usr_cb->DeviceSpeedDetected(phost->device_prop.speed);//log
  ...
  case HOST_USR_INPUT:    
    ...
    if(1)// phost->usr_cb->UserInput() == USBH_USR_RESP_OK)
    {
		phost->gState  = HOST_CLASS_INIT; 
		printf_usbh_core(" KEY  Pressed !!!!!!!!!!!!!!!!!!!!!!!! \n\n");  
		USB_OTG_BSP_mDelay(1000);
	}   
    break;
      
  case HOST_CLASS_INIT:    
    /*The function should return user response true to move to class state */
    if((phost->class_cb->Init(pdev, phost))\
        == USBH_OK)//USBH_DFU_InterfaceInit
      {
        phost->gState  = HOST_CLASS_REQUEST;    
		printf_usbh_core(" CLASS_INIT Over \n\r"); 
      }  
    break;
  ...
  case HOST_CTRL_XFER:
    //printf_usbh_core("\n\r process control transfer state machine");
    /* process control transfer state machine */
    USBH_HandleControl(pdev, phost);    
	if(phost->gState != HOST_CTRL_XFER){
		//printf_usbh_core(" state machine changed *******************************\n");
	}
    break;
...
/**
  ...
  * @retval USBH_Status
  */
static uint8_t DFU_INIT=0;//0:分配地址;1:不分配，使用默认地址0
static USBH_Status USBH_HandleEnum(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
  ...
  case ENUM_IDLE:  
    ...
    {
      printf_usbh_core("\n\r USBH_Get_DevDesc  only 1st 8 bytes");
  ...
  case ENUM_GET_CFG_DESC:  
                          ...
                          USB_CONFIGURATION_DESC_SIZE) == USBH_OK)
    {
      ...
      printf_usbh_core("\n\r get standard configuration descriptor  OK!\n");
    }
    break;
  ...
  case ENUM_GET_FULL_CFG_DESC:  
                         ...
                         phost->device_prop.Cfg_Desc.wTotalLength) == USBH_OK)
    {
      ...
      printf_usbh_core("\n\r get FULL config descriptor  OK!\n");
    }
    break;
  ...
  case ENUM_SET_CONFIGURATION:
	  if(DFU_INIT){
		phost->EnumState = ENUM_SET_RELIGION;
		break;
	  }
    ...
    {
      phost->EnumState = ENUM_SET_RELIGION;
      printf_usbh_core("\n\r set configuration OK!\n");
    }
    break;

  case ENUM_SET_RELIGION:
    /* set configuration  (default config) */
    //if (USBH_Get_CfgDesc(pdev, 
    //                     phost,
    //                     phost->device_prop.Cfg_Desc.wTotalLength) == USBH_OK)
    ...
    if ( USBH_Get_StringDesc(pdev,
                               phost,
                               0,//phost->device_prop.Dev_Desc.iManufacturer, 
                               (uint8_t*)&phost->device_prop.RELIGION_Desc, //Local_Buffer
                               0xff) == USBH_OK)

    {                          
        
      phost->EnumState = ENUM_DEV_CONFIGURED;
      if(phost->device_prop.RELIGION_Desc.Country == 0x0409){
        printf_usbh_core(" >语言国家为英国!\n");
      }
    }
    break;
