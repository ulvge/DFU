...
#define  LE24(addr)             (((u32)(*((u8 *)(addr))))\
                                + (((u32)(*(((u8 *)(addr)) + 1))) << 8)\
                                + (((u32)(*(((u8 *)(addr)) + 2))) << 16))&0x0fff
...
#define  USB_DESC_TYPE_Functional_DFU                      0x21
...
{
  uint8_t   bLength;            /*fixed 0x12*/
  uint8_t   bDescriptorType;    /*fixed 0x01*/    
...
{
  uint8_t   bLength;                /*fixed 0x09*/  
  uint8_t   bDescriptorType;      /*fixed 0x02*/  
...
typedef union _DFUDescriptor_Attributes 
{
  uint8_t d8;
  struct
  {
...
} USBH_DFUDesc_TypeDef_bit;

typedef struct _DFUDescriptor
{
  uint8_t   bLength;                /*  fixed     0x07     */
  uint8_t   bDescriptorType;       /*  fixed     0x21     */
  USBH_DFUDesc_TypeDef_bit	bmAttributes;
  
  uint16_t   wDetachTimeOut;        /* FFFFh 65535 ms */
  uint16_t   wTransferSize;        /* 1000h	4096 bytes */
}USBH_DFUDesc_TypeDef;

typedef struct _RELIGIONDescriptor
{
  uint8_t   bLength;                /*  fixed     0x04     */
  uint8_t   bDescriptorType;       /*  fixed     0x03 String     */
  uint16_t  Country;        /* 0409h English (United States) */
}USBH_RELDesc_TypeDef;

typedef struct _InterfaceDescriptor
{
  uint8_t bLength;              /*fixed 0x09*/
  uint8_t bDescriptorType;      /*fixed 0x04*/
...
{
  uint8_t   bLength;			/*fixed 0x07;*/
  uint8_t   bDescriptorType;	/*fixed 0x05;*/
  uint8_t   bEndpointAddress;   /* bit7,dirction;0:out,1:in;	indicates what endpoint this descriptor is describing */
