...
typedef struct _USB_OTG_GREGS  //000h
{
  ...
  __IO uint32_t GINTSTS;      /* Core Interrupt Register            014h    OTG_FS_GINTSTS*/
...
typedef union _USB_OTG_GINTMSK_TypeDef //P592
{
  uint32_t d32;
  struct
  {
...
typedef union _USB_OTG_GINTSTS_TypeDef //OTG_FS_GINTSTS p576
{
  uint32_t d32;
  struct
  {
uint32_t curmode ://当前模式 (Current mode of operation) 0：设备模式；1：主机模式
    1;
uint32_t modemismatch ://模式不匹配中断 (Mode mismatch interrupt)主从试图访问对方寄存器
    1;
uint32_t otgintr :	//OTG中断(OTG interrupt)
    1;
uint32_t sofintr :// (Start of frame)
    1;
uint32_t rxstsqlvl ://接收FIFO非空
    1;
uint32_t nptxfempty ://非周期性发送FIFO空 (Non-periodic TxFIFO empty)
    1;
uint32_t ginnakeff :
    1;
uint32_t goutnakeff :
...
uint32_t erlysuspend :
    1;
uint32_t usbsuspend ://onlyUSB挂起 (USB suspend)
    1;
uint32_t usbreset ://only USB复位 (USB reset)
    1;
uint32_t enumdone ://only  枚举完成 (Enumeration done)
    1;
uint32_t isooutdrop ://only 同步OUT包丢失中断
    1;
uint32_t eopframe ://only 从机 周期性帧结束中断 (End of periodic frame interrupt)
    1;
uint32_t Reserved16_17 :
    2;
uint32_t inepint://only 从机
    1;
uint32_t outepintr ://only 从机 OUT端点中断 
    1;
uint32_t incomplisoin ://only 从机
    1;
uint32_t incomplisoout ://主机:IPXFR：未完成的周期性传输 (Incomplete periodic transfer)
    1;                  //从机:incomplisoout：未完成的同步OUT传输
uint32_t Reserved22_23 :
    2;
uint32_t portintr ://主机端口中断 (Host port interrupt)
    1;
uint32_t hcintr ://主机通道中断 (Host channels interrupt)
    1;
uint32_t ptxfempty ://周期性发送FIFO空 (Periodic TxFIFO empty)
    1;
uint32_t Reserved27 :
    1;
uint32_t conidstschng ://Connector ID status change
    1;
uint32_t disconnect ://Disconnect detected
    1;
uint32_t sessreqintr ://Session request
    1;
uint32_t wkupintr : //Resume/remote wakeup detected interrupt
    1;
  }
  b;
...
typedef union _USB_OTG_HCINTn_TypeDef 
{
  ...
  struct
  {
uint32_t xfercompl ://传输完成(Transfer completed)
    1;
uint32_t chhltd :   //通道中止 (Channel halted)
    1;              //指示由于USB传输错误，或者应用程序中止请求导致的传输异常结束。    
uint32_t ahberr :   
    1;
uint32_t stall :    //STALL已收到中断 (STALL response received interrupt)
    1;
uint32_t nak :      //NAK已收到中断 (NAK response received interrupt)
    1;
uint32_t ack :      //ACK： ACK已收到/已发送中断 (ACK response received/transmitted interrupt)
    1;
uint32_t nyet :
    1;
uint32_t xacterr :      //传输错误 (Transaction error)  CRC校验失败/超时/位填充错误/EOP失败
    1;
uint32_t bblerr :       //串扰错误 (Babble error)
    1;
uint32_t frmovrun :     //数据PID错误 (Data toggle error)
    1;
uint32_t datatglerr :   //数据PID错误 (Data toggle error)
    1;
uint32_t Reserved :
    21;
  }
...
uint32_t nak ://NAK已收到中断 (NAK response received interrupt)
    1;
uint32_t ack ://ACK已收到/已发送中断 (ACK response received/transmitted interrupt)
    1;
uint32_t nyet :
    1;
uint32_t xacterr ://传输错误 (Transaction error)
    1;
uint32_t bblerr ://串扰错误 (Babble error)
    1;
uint32_t frmovrun ://帧溢出 (Frame overrun)
    1;
uint32_t datatglerr ://数据PID错误 (Data toggle error)
    1;
uint32_t Reserved :
