...
void  OSTimeDly (INT32U ticks)
{
    ...
    if (ticks > 0u) {                            /* 0 means no delay!                                  */
        ...
        OSRdyTbl[y] &= (OS_PRIO)~OSTCBCur->OSTCBBitX;  /* 将该组中所占Bit位清零                        */
        if (OSRdyTbl[y] == 0u) {                       /* 若该组中所有Bit位都为零，则该组・・・           */
            OSRdyGrp &= (OS_PRIO)~OSTCBCur->OSTCBBitY; /* ・・・没有就绪任务，将该组清除                  */
        }
...
*                             （唤醒被OSTimeDly()或OSTimeDlyHMSM()延时任务）
*
