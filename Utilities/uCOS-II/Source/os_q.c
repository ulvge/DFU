...
*                                       （创建一个消息队列）
*
* Description: 这个函数功能是创建一个消息队列（如果空ECB可用的情况下）。
*
* Arguments  : start         消息队列存储区域中，指向基地址的指针.
*                            存储区域必须被声明为一个 类型为 'void的数组指针。 如下：
*                            void *MessageStorage[size]
*
*              size          消息队列存储区域大小。
*
* Returns    : != (OS_EVENT *)0  返回指向分配给所创建的消息队列的事件控制块指针。
*              == (OS_EVENT *)0  没有事件控制块可用或检测到错误，返回空指针。
*********************************************************************************************************
*/
                      ...
                      INT16U    size)
{
    ...
    OS_ENTER_CRITICAL();
    pevent = OSEventFreeList;                    /* 得到下一个空ECB                                    */
    if (OSEventFreeList != (OS_EVENT *)0) {      /* 若还有空ECB(初始化时开辟出来的空间)・・              */
        OSEventFreeList = (OS_EVENT *)OSEventFreeList->OSEventPtr;   /* 使其指向下一空ECB              */
    }
    OS_EXIT_CRITICAL();
    if (pevent != (OS_EVENT *)0) {               /* 当前获得的ECB                                      */
        OS_ENTER_CRITICAL();
        pq = OSQFreeList;                        /* 获得空QCB                                          */
        if (pq != (OS_Q *)0) {                   /* 若得到非空QCB・・・                                   */
            OSQFreeList            = OSQFreeList->OSQPtr; /* 指向下一空QCB                             */
            OS_EXIT_CRITICAL();
                ...
                INT8U     *perr)
{
...
#if OS_ARG_CHK_EN > 0u
    if (pevent == (OS_EVENT *)0) {               /* 验证事件 'pevent'                                  */
        *perr = OS_ERR_PEVENT_NULL;
        return ((void *)0);
    }
#endif
    if (pevent->OSEventType != OS_EVENT_TYPE_Q) {/* 验证事件类型是否为 "消息类型"                      */
        *perr = OS_ERR_EVENT_TYPE;
        return ((void *)0);
    }
    if (OSIntNesting > 0u) {                     /* See if called from ISR ...                         */
    ...
    OS_ENTER_CRITICAL();
    pq = (OS_Q *)pevent->OSEventPtr;             /* 指向消息队列控制块QCB（当前事件）                  */
    if (pq->OSQEntries > 0u) {                   /* See if any messages in the queue                   */
        pmsg = *pq->OSQOut++;                    /* Yes, extract oldest message from the queue         */
        pq->OSQEntries--;                        /* Update the number of entries in the queue          */
        if (pq->OSQOut == pq->OSQEnd) {          /* Wrap OUT pointer if we are at the end of the queue */
...
{
#if OS_MAX_QS == 1u
    OSQFreeList         = &OSQTbl[0];                /* 只有一个队列时・・・                              */
    OSQFreeList->OSQPtr = (OS_Q *)0;
#endif

#if OS_MAX_QS >= 2u                                  /* 若大于一个队列时・・・                            */
    INT16U   ix;
    INT16U   ix_next;
    OS_Q    *pq1;
    OS_Q    *pq2;
    ...
    OS_MemClr((INT8U *)&OSQTbl[0], sizeof(OSQTbl));  /* 清除对列表                                     */
    for (ix = 0u; ix < (OS_MAX_QS - 1u); ix++) {     /* 初始化空QCB                                    */
        ix_next = ix + 1u;
        pq1 = &OSQTbl[ix];
        pq2 = &OSQTbl[ix_next];
        pq1->OSQPtr = pq2;
