...
#define  OS_GLOBALS         //定义OS_GLOBALS是为了应用于ucos_ii.h文件中(OS_EXT)全局变量的声明与定义
#include <ucos_ii.h>
#endif
...
*                                         （优先级查找表）
*
* Note: 变址索引表是位模式，找出就绪态最高优先级别任务。
*       给定值应符合高优先级位位值(0~7)。
*********************************************************************************************************
*/
//最高优先级任务查找表；对应OSRdyGrp和OSRbyTbl[i]的位值(0~7)
...
*                            （分配一个名称给信号量、互斥量、邮箱或队列）
*
* Description: This function assigns a name to a semaphore, mutex, mailbox or queue.
*
* Arguments  : pevent    指向事件组的指针.  'pevent' can point either to a semaphore,
...
* Returns    : none
*
* Notes      : 1) 调用这个函数前 中断应该禁止。
*              2) 你的ISR可以直接调用OSIntNesting, 因为OSIntNesting是全局变量。
*              3) 即使你直接对OSIntNesting加1， 你也必须调用OSIntExit()。
*              4) 你调用OSIntEnter()和OSIntExit()必须成对。
*                 也就是在你的ISR开始调用OSIntEnter()，结束时调用OSIntExit()。
*              5) 你的中断嵌套层数可达到255。
...
*                                            （退出ISR）
*
* Description: 这个函数使用率通知uC/OS-II已经完成了一个ISR.
*              当最后一个嵌套ISR完成后, 将调用uC/OS-II调度程序，确定是否有更高优先级的就绪任务。
*
* Arguments  : none
*
* Returns    : none
*
* Notes      : 1) 你调用OSIntEnter()和OSIntExit()必须成对。
*                 也就是在你的ISR开始调用OSIntEnter()，结束时调用OSIntExit()。
...
void  OSIntExit (void)
{
    ...
    if (OSRunning == OS_TRUE) {
        OS_ENTER_CRITICAL();
        if (OSIntNesting > 0u) {                           /* 如果中断嵌套层数大于0, 将其减1           */
            OSIntNesting--;
        }
        if (OSIntNesting == 0u) {                          /* 所有ISR完成...                           */
            if (OSLockNesting == 0u) {                     /* ...且没有上锁。                          */
...
*                                          （系统滴答函数）
*
* Description: 这个函数用于uC/OS-II发出"系统滴答"（也成为"时钟周期"）。
*              这个函数应该被"滴答ISR"调用， 同时也使一个高优先级任务就绪。
*
* Arguments  : none
*
* Returns    : none
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
#if OS_TICK_STEP_EN > 0u
        switch (OSTickStepState) {                         /* 决定我们是否要处理一个tick                   */
...
#endif
        ptcb = OSTCBList;                                  /* 指向TCB列表（最后创建的那个任务）            */
        while (ptcb->OSTCBPrio != OS_TASK_IDLE_PRIO) {     /* 查询所有的TCB列表, 直到IDLE任务              */
            OS_ENTER_CRITICAL();
            if (ptcb->OSTCBDly != 0u) {                    /* 若当前任务延时不等于0・・・                     */
                ptcb->OSTCBDly--;                          /* ・・・将这个任务延时值减1                       */
                if (ptcb->OSTCBDly == 0u) {                /* 若延时到・・・                                  */

                    if ((ptcb->OSTCBStat & OS_STAT_PEND_ANY) != OS_STAT_RDY) { /* PEND状态未就绪...        */
                        ptcb->OSTCBStat  &= (INT8U)~(INT8U)OS_STAT_PEND_ANY;   /* 清除PEND状态标志         */
                        ptcb->OSTCBStatPend = OS_STAT_PEND_TO;                 /* 表明等待超时             */
                    } else {
                        ptcb->OSTCBStatPend = OS_STAT_PEND_OK;                 /* 等待到来(等到完成)       */
                    }

                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* 若任务在就绪状态・・・      */
                        OSRdyGrp               |= ptcb->OSTCBBitY;             /* 使该任务就绪             */
                        OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
                    }
                }
            }
            ptcb = ptcb->OSTCBNext;                        /* 指向链表的后链接                             */
            OS_EXIT_CRITICAL();
        }
    }
}
...
*                                （获取事件就绪表最高优先级,使其运行）
*
* Description: 这个函数会被uC/OS-II其他函数调用来就绪事件等待列表。
*
* Arguments  : pevent      ECB指针。
*
*              pmsg        消息指针.  This pointer is used by message oriented services
                        ...
                        INT8U      pend_stat)
{
...
#if OS_LOWEST_PRIO <= 63u
    y    = OSUnMapTbl[pevent->OSEventGrp];              /* 查找高优先级任务(HPT) 等待消息              */
    ...
    ptcb                  =  OSTCBPrioTbl[prio];        /* 指向就绪任务控制块OS_TCB                    */
    ptcb->OSTCBDly        =  0u;                        /* 阻止任务延时                                */
#if ((OS_Q_EN > 0u) && (OS_MAX_QS > 0u)) || (OS_MBOX_EN > 0u)
    ptcb->OSTCBMsg        =  pmsg;                      /* Send message directly to waiting task       */
#else
    pmsg                  =  pmsg;                      /* Prevent compiler warning if not used        */
#endif
    ptcb->OSTCBStat      &= (INT8U)~msk;                /* 清除该事件状态                              */
    ptcb->OSTCBStatPend   =  pend_stat;                 /* Pend状态设置为post or abort                 */
                                                        /* See if task is ready (could be susp'd)      */
    if ((ptcb->OSTCBStat &   OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* 若任务状态未处于暂停状态...        */
        OSRdyGrp         |=  ptcb->OSTCBBitY;           /* 使任务进入就绪列表                          */
        OSRdyTbl[y]      |=  ptcb->OSTCBBitX;
    }

    OS_EventTaskRemove(ptcb, pevent);                   /* 从事件等待列表中移除这个等待的事件          */
...
*                                      （使任务等待事件的发生）
*
* Description: 这个函数被uC/OS-II调用来暂停任务，使其等待某事件的发生。
*
* Arguments  : pevent   指向等待事件控制块ECB的指针。
*
* Returns    : none
*
...
void  OS_EventTaskWait (OS_EVENT *pevent)
{
    ...
    OSTCBCur->OSTCBEventPtr               = pevent;                 /* 该任务事件指针指向的事件        */

    pevent->OSEventTbl[OSTCBCur->OSTCBY] |= OSTCBCur->OSTCBBitX;    /* 让任务进入事件等待列表          */
    pevent->OSEventGrp                   |= OSTCBCur->OSTCBBitY;

    y             =  OSTCBCur->OSTCBY;                              /* 使当前任务不再处于就绪态        */
    OSRdyTbl[y]  &= (OS_PRIO)~OSTCBCur->OSTCBBitX;
    if (OSRdyTbl[y] == 0u) {                                        /* 若该组中没有就绪任务了...       */
        OSRdyGrp &= (OS_PRIO)~OSTCBCur->OSTCBBitY;                  /* ...则清除该组                   */
    }
}
#endif
/*$PAGE*/
...
*                                      （初始化等待的ECB）
*
* Description: 这个函数会被uC/OS-II其他函数调用来初始化事件等待列表。
*
* Arguments  : pevent    指向ECB分配出来的事件。
*
* Returns    : none
*
* Note       : 这个函数是uC/OS-II系统内部调用的函数，你的应用程序不能调用它。
...
void  OS_EventWaitListInit (OS_EVENT *pevent)
{
    ...
    pevent->OSEventGrp = 0u;                     /* 在事件中没有任务等待                               */
...
*                                          （初始化事件控制块）
*
* Description: 这个函数会被OSInit()调用来初始化事件控制块ECB。
*
* Arguments  : none
*
* Returns    : none
...
static  void  OS_InitEventList (void)
{
    ...
    for (ix = 0u; ix < (OS_MAX_EVENTS - 1u); ix++) {        /* 初始化空ECB                             */
        ix_next = ix + 1u;
...
static  void  OS_InitTCBList (void)
{
    ...
    ptcb1                   = &OSTCBTbl[ix];
    ptcb1->OSTCBNext        = (OS_TCB *)0;                       /* 末尾 OS_TCB                        */
#if OS_TASK_NAME_EN > 0u
    ptcb1->OSTCBTaskName    = (INT8U *)(void *)"?";              /* Unknown name                       */
#endif
...
*                                          （清除一段内存）
*
* Description: 这个函数会被uC/OS-II其他服务程序调用来清楚一段连续的内存RAM。
*
* Arguments  : pdest    要清除内存的首地址(i.e. write 0x00 to)。
*
*              size     要清除的字节数。
*
* Returns    : none
*
* Notes      : 1) 这个函数是uC/OS-II系统内部调用的函数，你的应用程序不能调用它。
*              2) 我们只能清楚64K字节内存。
*                 这个不是问题，因为清楚的内存基本没有接近这么大。
...
*                                              任务调度
*
* Description: 这个函数会被uC/OS-II其他函数调用来确定新的最高优先级就绪任务, 使其运行。
*              This function is invoked by TASK level code and is not used to reschedule tasks
...
*
* Notes      : 1) 这个函数是uC/OS-II系统内部调用的函数，你的应用程序不能调用它。
*              2) 给调度器上锁，用于禁止任务调度 (查看OS_SchedLock()函数)。
*********************************************************************************************************
*/

void  OS_Sched (void)
{
#if OS_CRITICAL_METHOD == 3u                           /* 临界区为模式3, 则给CPU状态寄存器分配存储空间 */
    OS_CPU_SR  cpu_sr = 0u;
#endif


    OS_ENTER_CRITICAL();                               /* 进入临界区（关中断）                         */
    if (OSIntNesting == 0u) {                          /* 中断嵌套层数=0, 没有ISR运行                  */
        if (OSLockNesting == 0u) {                     /* 嵌套计数器未上锁（调度器未上锁）             */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      */
#endif
                OSCtxSwCtr++;                          /* 任务切换次数(统计计数器)+1,以跟踪任务切换次数*/
...
*                                   （查找最高优先级任务进入就绪）
*
* Description: 这个函数会被uC/OS-II其他服务函数调用来确定最高优先级就绪任务。
*              全局变量'OSPrioHighRdy' 相应的改变。
*
* Arguments  : none
*
* Returns    : none
*
* Notes      : 1) 这个函数是uC/OS-II系统内部调用的函数，你的应用程序不能调用它。
*              2) 当你调用这个函数时，中断需设定为失能。
*********************************************************************************************************
*/

static  void  OS_SchedNew (void)
{
#if OS_LOWEST_PRIO <= 63u                        /* 如果我们的任务小于64个                             */
    INT8U   y;
...
*                                        （初始化任务控制块）
*
* Description: 这个uC/OS-II内部函数， 用于创建任务时初始化TCB (查看OSTaskCreate() and OSTaskCreateExt()).
*
...
*
* Returns    : OS_ERR_NONE              没错误，调用成功。
*              OS_ERR_TASK_NO_MORE_TCB  没有更多的任务控制块被分配，将无法创建新的任务。
*
* Note       : 这个函数是uC/OS-II系统内部调用的函数，你的应用程序不能调用它。
*********************************************************************************************************
*/
...
INT8U  OS_TCBInit (INT8U    prio,
                   ...
                   INT16U   opt)
{
    OS_TCB    *ptcb;                                 /* 定义一个TCB结构的指针（用于当前TCB）           */
    ...
    if (ptcb != (OS_TCB *)0) {                             /* 如果缓冲区有空余TCB，这个TCB将被初始化   */
...
#if OS_TASK_PROFILE_EN > 0u
        ptcb->OSTCBCtxSwCtr      = 0uL;                    /* 初始化分析变量                           */
        ...
        OSTCBInitHook(ptcb);                               /* TCB初始化HOOK函数                        */
        ...
        OS_ENTER_CRITICAL();
        ptcb->OSTCBNext = OSTCBList;                       /* TCB链表后连接指向OSTCBList               */
        ptcb->OSTCBPrev = (OS_TCB *)0;
        if (OSTCBList != (OS_TCB *)0) {                    /* 如果不是首次创建任务・・・                  */
            OSTCBList->OSTCBPrev = ptcb;
        }
        OSTCBList               = ptcb;                    /* TCB列表（也即最后创建的任务）            */
        OSRdyGrp               |= ptcb->OSTCBBitY;         /* 使该任务准备就绪（进入就绪态）           */
        OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
        OSTaskCtr++;                                       /* 任务计数器 + 1                           */
        OS_EXIT_CRITICAL();
        return (OS_ERR_NONE);                              /* TCB初始化成功，返回OS_ERR_NONE           */
    }
    OS_EXIT_CRITICAL();
