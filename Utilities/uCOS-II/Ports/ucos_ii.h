...
#ifdef   OS_GLOBALS                                     /* OS_GLOBALS定义于os_core.c文件中             */
#define  OS_EXT                                         /* OS_EXT定义为 "空"                           */
#else
...
#define  OS_N_SYS_TASKS                 2u              /* 系统任务数量                                */
#else
...
#define  OS_RDY_TBL_SIZE   ((OS_LOWEST_PRIO) / 8u + 1u) /* 就绪表大小                                  */
#else
...
*********************************************************************************************************
*/
#define  OS_EVENT_TYPE_UNUSED           0u  /* 事件未用                                                */
#define  OS_EVENT_TYPE_MBOX             1u  /* 事件类型为消息邮箱                                      */
#define  OS_EVENT_TYPE_Q                2u  /* 事件类型为消息队列                                      */
#define  OS_EVENT_TYPE_SEM              3u  /* 事件类型为信号量                                        */
#define  OS_EVENT_TYPE_MUTEX            4u  /* 事件类型为互斥量                                        */
#define  OS_EVENT_TYPE_FLAG             5u  /* 事件类型为事件标志                                      */

#define  OS_TMR_TYPE                  100u  /* 用于定时器类型...                                       */
...
typedef struct os_event {
    INT8U    OSEventType;                   /* ECB类型 (查看OS_EVENT_TYPE_xxxx)                        */
    void    *OSEventPtr;                    /* 指向消息或队列的指针                                    */
    INT16U   OSEventCnt;                    /* 信号量计数器 (只适用于信号量！若非信号量类型，则无用)   */
    OS_PRIO  OSEventGrp;                    /* 等待任务发生组                                          */
...
typedef struct os_flag_grp {                /* 事件标志组                                              */
    INT8U         OSFlagType;               /* 事件标志类型, 应设置为: OS_EVENT_TYPE_FLAG              */
    void         *OSFlagWaitList;           /* 事件标志组等待列表,用于连接OSFlagTbl[]                  */
    OS_FLAGS      OSFlagFlags;              /* 标志位数. 可以为:8, 16 or 32                            */
...
typedef struct os_mbox_data {
    void   *OSMsg;                          /* 指向消息邮箱中的消息。                                  */
    OS_PRIO OSEventTbl[OS_EVENT_TBL_SIZE];  /* 任务中等待事件发生的列表。                              */
    OS_PRIO OSEventGrp;                     /* 任务中等待事件发生的组。                                */
} OS_MBOX_DATA;
#endif
...
#if OS_Q_EN > 0u
typedef struct os_q {                       /* 队列控制块 QCB                                          */
    struct os_q   *OSQPtr;                  /* 连接到下一个队列控制块指针                              */
    void         **OSQStart;                /* 指向开始队列数据的指针                                  */
    void         **OSQEnd;                  /* 指向结束队列数据的指针                                  */
    void         **OSQIn;                   /* 在队列中，指向下一个消息将要插入地方的指针              */
    void         **OSQOut;                  /* 在队列中，指向下一个消息将要被提取地方的指针            */
    INT16U         OSQSize;                 /* 队列大小 (maximum number of entries)                    */
    INT16U         OSQEntries;              /* 当前进入队列的数量                                      */
} OS_Q;
...
typedef struct os_sem_data {
    INT16U  OSCnt;                          /* 信号量计数值                                            */
    OS_PRIO OSEventTbl[OS_EVENT_TBL_SIZE];  /* 任务等待事件发生的列表                                  */
    OS_PRIO OSEventGrp;                     /* 等待事件组                                              */
} OS_SEM_DATA;
#endif
...
typedef struct os_tcb {
    ...
    INT32U           OSTCBStkSize;          /* 任务堆栈的容量(in number of stack elements)             */
    ...
    INT32U           OSTCBCtxSwCtr;         /* 一定时间进行任务切换                                    */
...
typedef  struct  os_tmr {
    INT8U            OSTmrType;             /* 应该设置为OS_TMR_TYPE类型                               */
    OS_TMR_CALLBACK  OSTmrCallback;         /* 当定时到时，调用的函数                                  */
    void            *OSTmrCallbackArg;      /* 当定时到时，传递给调用函数的参数                        */
    void            *OSTmrNext;             /* 双向链表指针 Next                                       */
    void            *OSTmrPrev;             /* 双向链表指针 Prev                                       */
    INT32U           OSTmrMatch;            /* 当OSTmrTime == OSTmrMatch， 定时到                      */
    INT32U           OSTmrDly;              /* 周期更新前延时时间                                      */
    INT32U           OSTmrPeriod;           /* 定时周期                                                */
#if OS_TMR_CFG_NAME_EN > 0u
    INT8U           *OSTmrName;             /* 定时器名称                                              */
#endif
    INT8U            OSTmrOpt;              /* 选项 (查看OS_TMR_OPT_xxx)                               */
    INT8U            OSTmrState;            /* 定时器状态：                                            */
                                            /*  未用：OS_TMR_STATE_UNUSED                              */
                                            /*  运行：OS_TMR_STATE_RUNNING                             */
                                            /*  暂停：OS_TMR_STATE_STOPPED                             */
} OS_TMR;
...
OS_EXT  INT32U            OSCtxSwCtr;               /* 上下文切换次数（统计计数器）                    */

#if (OS_EVENT_EN) && (OS_MAX_EVENTS > 0u)
OS_EXT  OS_EVENT         *OSEventFreeList;          /* 指向空ECB列表的指针                             */
OS_EXT  OS_EVENT          OSEventTbl[OS_MAX_EVENTS];/* ECB表                                           */
#endif

#if (OS_FLAG_EN > 0u) && (OS_MAX_FLAGS > 0u)
OS_EXT  OS_FLAG_GRP       OSFlagTbl[OS_MAX_FLAGS];  /* 事件标志数组                                    */
OS_EXT  OS_FLAG_GRP      *OSFlagFreeList;           /* 事件标志空指针                                  */
#endif
...
OS_EXT  INT8U             OSLockNesting;            /* 多任务嵌套计数器锁                              */

OS_EXT  INT8U             OSPrioCur;                /* 当前任务优先级                                  */
OS_EXT  INT8U             OSPrioHighRdy;            /* 就绪任务的最高优先级                            */

OS_EXT  OS_PRIO           OSRdyGrp;                 /* 就绪表组,每i位对应OSRdyTbl[i]组有任务就绪0~7    */
OS_EXT  OS_PRIO           OSRdyTbl[OS_RDY_TBL_SIZE];  /* 对应就绪表组值, 也是优先级最高的任务          */

OS_EXT  BOOLEAN           OSRunning;                       /* 运行标志。OSRunning == 1时运行           */

OS_EXT  INT8U             OSTaskCtr;                       /* 任务计数器                               */

OS_EXT  volatile  INT32U  OSIdleCtr;                       /* 空闲任务计数器                           */

#ifdef OS_SAFETY_CRITICAL_IEC61508
OS_EXT  BOOLEAN           OSSafetyCriticalStartFlag;
#endif

OS_EXT  OS_STK            OSTaskIdleStk[OS_TASK_IDLE_STK_SIZE]; /* 空闲任务堆栈栈顶指针                */


OS_EXT  OS_TCB           *OSTCBCur;                        /* 指向当前运行的TCB指针                    */
OS_EXT  OS_TCB           *OSTCBFreeList;                   /* 游离TCB列表指针（指向OSTCBTbl[]）        */
OS_EXT  OS_TCB           *OSTCBHighRdy;                    /* TCB最高优先级指针                        */
OS_EXT  OS_TCB           *OSTCBList;                       /* TCB双向链表指针                          */
OS_EXT  OS_TCB           *OSTCBPrioTbl[OS_LOWEST_PRIO + 1u];        /* TCB优先级表                     */
...
OS_EXT  OS_Q              OSQTbl[OS_MAX_QS];        /* QCB表                                           */
#endif
...
OS_EXT  INT16U            OSTmrUsed;                /* 定时器使用的数量                                */
OS_EXT  INT32U            OSTmrTime;                /* 当前定时器Time                                  */

OS_EXT  OS_EVENT         *OSTmrSem;                 /* Sem. used to gain exclusive access to timers    */
OS_EXT  OS_EVENT         *OSTmrSemSignal;           /* Sem. 用于信号量更新定时器                       */

OS_EXT  OS_TMR            OSTmrTbl[OS_TMR_CFG_MAX]; /* 定时器列表                                      */
OS_EXT  OS_TMR           *OSTmrFreeList;            /* 指向空定时器的指针                              */
OS_EXT  OS_STK            OSTmrTaskStk[OS_TASK_TMR_STK_SIZE];       /* 定时器任务堆栈大小              */
