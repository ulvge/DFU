...
*                                            (BEGINNING)
*
* Description: 这个函数被OSInit()调用。
*
* Arguments  : none
*
* Note(s)    : 1) 在调用这个函数期间应该禁止中断。
*********************************************************************************************************
*/
...
*                                         （初始化一个任务堆栈）
*
* Description: 初始化被创建任务的堆栈。这个函数被OSTaskCreate() 或 OSTaskCreateExt()调用。
*              这个函数非常特定于处理器。
*
...
OS_STK *OSTaskStkInit (void (*task)(void *p_arg), void *p_arg, OS_STK *ptos, INT16U opt)
{
    ...
    *(--stk)  = (INT32U)0x04040404uL;            /* R4                                                 */
    ...
    return (stk);                                /* 返回新栈顶（也是指向分配出來的堆栈空间）           */
}
...
/* 注释下面两函数 OS_CPU_SysTickHandler() 和 OS_CPU_SysTickInit() ，此处我们将引用ST库提供的相应函数*/
#if 0
/*
*********************************************************************************************************
*                                          SYS TICK HANDLER
